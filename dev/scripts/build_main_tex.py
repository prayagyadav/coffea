...
parser.add_argument("-i", "--indir", help="Enter the export directory path", type=str)
...
with open(template_file) as tex:
    data = tex.read()
...
for f in tex_files:
    ...
    b = (
        a.strip()
        .strip("\\begin{frame}[allowframebreaks,fragile]")
        .strip("\\end{frame}")
        .strip()
        .strip("\\pagebreak")
        .strip()
    ...
    content = (
        b.strip("title").strip("% Add a bibliography block to the postdoc").strip()
    )
