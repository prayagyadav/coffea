...
_trailing_under = re.compile(r".*_[0-9]") # Any name with a trailing underscore and an integer _n
...
class FCCSchema(BaseSchema):
    ...
    event_ids = ["col_metadata", "evt_metadata", "metadata", "run_metadata",]
    ...
    _threevec_fields = {
        ...
        "momentumAtEndpoint":["momentumAtEndpoint.x","momentumAtEndpoint.y","momentumAtEndpoint.z"],
    ...
    def _idx_collections(self, output, branch_forms, all_collections):
        ...
        #Remove grouping branches which are generated from BaseSchema and contain no useable info
        _grouping_branches = {
    ...
    def _main_collections(self, output, branch_forms, all_collections):
        ...
        for name in collections:
            ...
            output[name] = zip_forms(
                sort_dict(collection_content), name, mixin
            )
    ...
    def _trailing_underscore_collections(self, output, branch_forms, all_collections):
        ...
        singletons = {
            ...
            if _trailing_under.match(collection_name) and not _all_collections.match(collection_name)
        }
    ...
    def _create_subcollections(self, branch_forms, all_collections):
        ...
        for name in all_collections:
            for threevec_name, subfields in self._threevec_fields.items():
                if all(f"{name}/{name}.{subfield}" in field_names for subfield in subfields):
                    content = {
                    ...
                    branch_forms[f"{name}/{name}.{threevec_name}"] = zip_forms(sort_dict(content), threevec_name, "ThreeVector")
        return branch_forms
    ...
    def _build_collections(self, field_names, input_contents):
        ...
        output, branch_forms = self._idx_collections(output, branch_forms, all_collections)

        output, branch_forms = self._trailing_underscore_collections(output, branch_forms, all_collections)

        output, branch_forms = self._main_collections(output, branch_forms, all_collections)

        output, branch_forms = self._unknown_collections(output, branch_forms, all_collections)
